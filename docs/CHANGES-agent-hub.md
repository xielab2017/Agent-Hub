...
## 10. 账号登录移到「多模型 API」，作为模型来源（v5.5.0）

- Claude 订阅（Claude Code）、ChatGPT（Codex）、Cursor 的安装、登录、权限都在「多模型 API」页，与各家 API 并列；可同时登录多个账号。
- 「快捷登录」：一键——未安装先自动安装，再自动在新标签页打开官方授权页；登录后在卡片上点「用于办公 / 推理 / 全部对话」即可。
- 分级路由可以把某一级指给某个账号：该级的对话由对应 Agent 回答（用该级设置的模型，不用输入框的模型）；问候、任务步骤和 Hub 内部调用（Skill、检索词、Hub Agent）走最近的 API 厂商。
- Claws 面板只保留 Hermes / OpenClaw 等 claw。

## 11. 统一模型选择 + 多选融合回答（v5.5.0）

- 输入框旁的「模型」按钮列出所有可用模型：各家 API 的模型（已获取 / 推荐 / 已绑定）与已登录的账号（Claude opus / sonnet / haiku、ChatGPT(Codex)、Cursor），统一为 `厂商::模型`。
- 勾一个：只用它回答（用它自己的地址和 key，不受当前后端影响）；勾多个：「融合回答」——同时作答、共享同一份检索证据与工作区上下文，再由一个 API 模型合并成一份回答，保留共识、写明分歧；每个模型的原始回答可在下方展开。
- 某个模型失败会被记录，其余照常融合；问候和任务步骤只用第一个所选模型；选择会被记住。
