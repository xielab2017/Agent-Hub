...
def build_grounded_preamble(
    ...
    *,
    ...
    # Code-context detection: when the user attached R / Python / notebook
    # files, build a small profile (language, packages, intent hints) so the
    # LLM anchors its review on the actual API surface the student is
    # using, instead of guessing.  Runs only when at least one excerpt
    # looks like a script — pure CSVs / Excel don't trigger it.
    code_block = ""
    try:
        from .code_context import build_code_context, format_code_context_md
        attachments_for_ctx: list[dict[str, Any]] = []
        for e in excerpts or []:
            ...
            if not content:
                continue
            name = str(e.get("relative") or e.get("path") or "")
            # Trigger when the file extension is a known code type, or
            # when `detect_language` can identify the body on its own
            # (e.g. a pasted snippet in chat).
            from .code_context import detect_language
            has_code_ext = any(name.lower().endswith(ext) for ext in (
                ".r", ".rmd", ".py", ".ipynb", ".jl", ".sh", ".bash",
                ".smk", ".nf", ".snakefile", ".sql",
            ))
            detected_lang = detect_language(name, content)
            if not (has_code_ext or detected_lang):
                continue
            attachments_for_ctx.append({
                "name": name,
                "text": content,
                "language": detected_lang,
                "path": e.get("path") or "",
            })
        if attachments_for_ctx:
            ctx = build_code_context(attachments_for_ctx)
            if ctx.get("languages"):
                code_block = format_code_context_md(ctx, lang="zh")
