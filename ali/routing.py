...
def resolve_route(
    ...
    if use_hybrid:
        ...
        model = coerce_model_for_provider(provider_id, model, route_key=route_key)
        # Provider catalog is canonical. Replace stale URLs/env names left by
    ...
    if (
        ...
        selected_provider = (
            backend_type
            if backend_type not in ("", "hybrid") and not use_hybrid
            else (configured_provider or provider_id or backend_type)
        ).strip()
        selected_model = str(tier_entry.get("model") or "").strip()
        if (
            backend_type not in ("", "hybrid")
            and not use_hybrid
            and configured_provider
            and configured_provider != backend_type
        ):
