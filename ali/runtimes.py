...
_CLAW_PROVIDER_MAP = {
    ...
    # endpoint, so all three downstream Claws end up talking "openai" with
    # the right base_url + DASHSCOPE_API_KEY.
    "dashscope": "openai",
    "aliyun": "openai",
    "alibaba": "openai",
    "qwen": "openai",
...
def _resolve_hub_llm(cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    Order of preference:
      ...
      1. The Control Center "Models" tab slots (legacy) — ``models.main``
         and ``models.fast`` still win when set, because a lot of older
         configs only fill these.
      2. The Control Center "Routing" tab per-tier bindings
         (``routing.tier_models.C1`` and ``C2`` are the office + long-form
         tiers the Hub actually uses for normal chat). This is what the
         user just changed in the screenshot — the binding *must* be
         honoured, otherwise a fresh "dashscope · qwen3.7-max-preview"
         config would silently fall through to the stale ``models.main``
         value (which is how we ended up with
         ``provider=custom model=deepseek-r1-distill-llama-70b`` in
         the Sync to Hermes result).
      3. A fresh ``backend`` / ``hybrid`` fallback for the very first
         sync before any tier binding has been set.
    """
    ...
    # Per-tier bindings win over the legacy slot fields when they are
    # populated. We probe C1 (office) and C2 (long-form) in that order
    # because those are the tiers the Hub actually dispatches against.
    routing = cfg.get("routing") or {}
    tier_models = routing.get("tier_models") or {}
    tier_provider = ""
    tier_model = ""
    if isinstance(tier_models, dict):
        for tier_key in ("C1", "C2", "C0", "C3", "Vision"):
            entry = tier_models.get(tier_key) or {}
            if not isinstance(entry, dict):
                continue
            cand_provider = str(entry.get("provider") or "").strip()
            cand_model = str(entry.get("model") or "").strip()
            if not cand_model:
                continue
            # Skip the "auto" sentinel that the UI emits when the tier
            # is left on "follow route" — we want an explicit binding.
            if cand_provider in ("", "auto"):
                continue
            tier_provider = cand_provider
            tier_model = cand_model
            break
    ...
    # Resolve the model. Tier binding wins; legacy slots are the fallback.
    use_model = tier_model or str(
        models.get("main") or models.get("qwen_main") or models.get("fast") or ""
    ).strip()

    # If the tier binding names a provider that differs from the backend
    # default, prefer it — but only when the named provider is one we
    # actually have keys for, so we never overwrite a working
    # deepseek key with an empty dashscope one.
    if tier_provider and tier_provider != pid:
        tier_key = resolve_api_key(cfg, provider=tier_provider)
        if (tier_key.get("key") or "").strip() or tier_provider == pid:
            pid = tier_provider
            # Re-resolve env_name + base_url + api_key for the new
            # provider so the Hermes .env / config.yaml get the right
            # vendor block.
            env_name = str(tier_key.get("env_name") or env_name).strip()
            tier_key_value = str(tier_key.get("key") or "").strip()
            if tier_key_value:
                api_key = tier_key_value
            try:
                from .providers import get_provider as _get_prov
                tier_prov_meta = _get_prov(tier_provider) or {}
            except Exception:  # noqa: BLE001
                tier_prov_meta = {}
            tier_base_url = str(tier_prov_meta.get("base_url") or "").strip()
            if tier_base_url:
