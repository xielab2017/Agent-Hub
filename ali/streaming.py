...
def _put(q: queue.Queue, event: str, data: dict[str, Any] | None = None) -> None:
    ...
    # Stamp before enqueue so SSE clients always see duration on terminal events.
...
def start_chat(
    ...
    if active_sub and not (soul_role or "").strip():
        try:
            ...
            soul_mod.set_active_role(
                str(active_sub.get("soul_role") or active_sub.get("role") or "office"),
    ...
    route_info["soul_role"] = str(
        (soul_role or "").strip()
        or (ali or {}).get("active_soul_role")
        or "office"
    )
    if active_sub:
...
def _run_agent_streaming(
    ...
    try:
        ...
        # Illustrative code paths are ignored; UI may show a non-blocking footer.
