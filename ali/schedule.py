...
def _load() -> dict[str, Any]:
    path = _tasks_file()
    if not path.is_file():
        return {"tasks": [], "defaults": {"nightly_hour": 0, "maintenance_hour": 2, "morning_hour": 7}}
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        if isinstance(data, dict):
            data.setdefault("tasks", [])
            data.setdefault("defaults", {"nightly_hour": 0, "maintenance_hour": 2, "morning_hour": 7})
            return data
    ...
    return {
        ...
        "builtins": [
            ...
            },
            {
                ...
                "kind": "maintenance",
                "label": "夜间自治维护",
                "label_en": "Nightly maintenance",
                "hour": int((data.get("defaults") or {}).get("maintenance_hour", 2)),
                "minute": 0, "enabled": True, "builtin": True,
            },
            {
                "id": "builtin-morning",
                "kind": "evolve",
...
def save_defaults(nightly_hour: int | None = None, morning_hour: int | None = None, maintenance_hour: int | None = None) -> dict[str, Any]:
    data = _load()
    ...
    if maintenance_hour is not None:
        defaults["maintenance_hour"] = max(0, min(23, int(maintenance_hour)))
