...
def create_folder(name: str) -> dict[str, Any]:
    ...
    now = time.time()
    item = {"id": str(uuid.uuid4()), "name": label[:80], "created_at": now, "updated_at": now, "sort_order": len(_load())}
    with _lock:
        items = _load(); items.append(item); _save(items)
    return item

def update_folder(folder_id: str, name: str | None = None, sort_order: int | None = None) -> dict[str, Any] | None:
    with _lock:
        items = _load()
