...
def handle_get(handler) -> None:
    ...
    if path == "/api/search":
        ...
        return _json(handler, 200, websearch.search_web(q, limit=max(1, min(limit, 16)), deep=False))
...
def handle_post(handler) -> None:
    ...
    if path == "/api/schedule":
        body = _read_json(handler)
        try:
            if body.get("defaults") is not None or "nightly_hour" in body or "morning_hour" in body:
                return _json(
                    handler,
                    200,
    ...
    if path == "/api/agents/upsert":
        body = _read_json(handler)
        try:
            sub = body.get("subagent") if isinstance(body.get("subagent"), dict) else body
            result = agents.upsert_subagent(sub if isinstance(sub, dict) else {})
            audit.log_event("agents_upsert", {"id": (sub or {}).get("id") if isinstance(sub, dict) else ""})
            return _json(handler, 200, result)
        except ValueError as exc:
            return _json(handler, 400, {"error": str(exc)})
