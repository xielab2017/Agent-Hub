...
POPULAR_AGENT_MODELS = {
    "cn": [
        ...
        {"id": "minimax-m3", "provider": "minimax-cn", "role": "main", "label": "MiniMax M3", "docs": "https://platform.minimaxi.com/"},
...
def _fetch_github_hot(limit: int = 8) -> tuple[list[dict[str, Any]], str]:
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            fut = pool.submit(skills_hub.fetch_github_trending_skills, limit, timeout=10.0)
        ...
        return skills_hub.curated_skill_fallback(limit), "curated"
...
def start_scheduler() -> None:
    ...
    def _hours() -> tuple[int, int, int]:
        try:
            from .schedule import _load as load_sched

            d = (load_sched().get("defaults") or {})
            return int(d.get("nightly_hour", 0)), int(d.get("maintenance_hour", 2)), int(d.get("morning_hour", 7))
        except Exception:  # noqa: BLE001
            return 0, 2, 7
    ...
    def _loop() -> None:
        while True:
            try:
                ...
                key_a = f"maintenance:{now.date().isoformat()}"
                if now.hour == nh and now.minute < 5 and st.get("nightly") != key_n:
                    run_nightly()
                    st["nightly"] = key_n
                    _save(st)
                if now.hour == ah and now.minute < 5 and st.get("maintenance") != key_a:
                    from . import nightly_maintenance
                    nightly_maintenance.run()
                    st["maintenance"] = key_a
                    _save(st)
