...
_PROVIDER_MAP = {
    ...
    # speaks OpenAI-compat when we give it the right base_url and
    # DASHSCOPE_API_KEY. Mapping to "custom" routes through the
    # _write_managed_env / _patch_model_yaml path, which already wires
    # OPENAI_BASE_URL + base provider block.
    "dashscope": "custom",
    "aliyun": "custom",
    "alibaba": "custom",
    "qwen": "custom",
...
def sync_hub_to_hermes(
    ...
    *,
    ...
    # Resolve the model. The Control Center Routing tab can pin a per-tier
    # Provider + Model binding (e.g. dashscope · qwen3.7-max-preview for
    # C1 office). When that binding is set, it must win over the legacy
    # ``models.main`` / ``models.fast`` slot fields — otherwise a fresh
    # routing config silently falls through to a stale slot value, which
    # is how the Hermes .env ended up with model=deepseek-r1-distill-llama-70b
    # while the UI proudly showed qwen3.7-max-preview.
    routing = cfg.get("routing") or {}
    tier_models = routing.get("tier_models") or {}
    tier_model = ""
    tier_provider_override = ""
    if isinstance(tier_models, dict):
        for tier_key in ("C1", "C2", "C0", "C3", "Vision"):
            ...
            if not cand_model or cand_provider in ("", "auto"):
                continue
            tier_model = cand_model
            tier_provider_override = cand_provider
            break
    ...
    # default, prefer it — but only when we actually have a key for the
    # named provider, so we never overwrite a working key with an empty
    # one. This keeps Hermes pointed at the provider the user just chose.
    if tier_provider_override and tier_provider_override != pid:
        tier_key = resolve_api_key(cfg, provider=tier_provider_override)
        if (tier_key.get("key") or "").strip():
            pid = tier_provider_override
            ...
            try:
                ...
                tier_prov_meta = _get_prov(tier_provider_override) or {}
            except Exception:  # noqa: BLE001
                tier_prov_meta = {}
